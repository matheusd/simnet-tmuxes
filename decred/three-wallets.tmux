#!/bin/sh
#
# Script to setup parallel dcrd nodes with separate wallets and a third empty
# wallet.
# Useful for testing reorgs by disconnecting nodes, mining individually, then
# reconnecting them and testing the effect of this on a given arbitrary wallet
# of the network.
#
#             alpha  <------>  beta
#    listen   19100           19200
...
WALLET03_SEED="3333333333333333333333333333333333333333333333333333333333333333"
...
mkdir -p "${NODES_ROOT}/"{alpha,beta,w-alpha,w-beta,w-gamma}
...
cp ${NODES_ROOT}/dcrctl.conf ${NODES_ROOT}/w-gamma
cat >> ${NODES_ROOT}/w-gamma/dcrctl.conf <<EOF
walletrpcserver = 127.0.0.1:19302
EOF
...
cp ${NODES_ROOT}/w-alpha/ctl ${NODES_ROOT}/w-gamma
cp ${NODES_ROOT}/w-alpha/tickets ${NODES_ROOT}/w-gamma
cp ${NODES_ROOT}/w-alpha/xfer ${NODES_ROOT}/w-gamma
...
--enableticketbuyer --rpclisten 127.0.0.1:19102" C-m
...
--enableticketbuyer --rpclisten 127.0.0.1:19202" C-m
tmux select-pane -t 3
tmux send-keys "cd w-beta" C-m



tmux new-window -t $SESSION:4 -n 'gamma'
tmux split-window -v
tmux select-pane -t 0
tmux send-keys "cd w-gamma" C-m
tmux send-keys "dcrwallet -C ../wallet.conf --create" C-m
sleep 2
tmux send-keys "123" C-m "123" C-m "n" C-m "y" C-m
sleep 1
tmux send-keys "${WALLET03_SEED}" C-m C-m
tmux send-keys "dcrwallet -C ../wallet.conf --rpcconnect 127.0.0.1:19201 \
--rpclisten 127.0.0.1:19302 --debuglevel WLLT=DEBUG" C-m
tmux select-pane -t 1
tmux send-keys "cd w-gamma" C-m
